...
public class MultiTouch {
	...
	public void mTouchAction() throws InterruptedException {
		...
		TouchAction action1 = new TouchAction((MobileDriver) driver).longPress(
				x1, y1).waitAction();
		// Set touch action2 on given X Y Coordinates of screen.
		TouchAction action2 = new TouchAction((MobileDriver) driver).longPress(
				x2, y2).waitAction();
		// Set touch action3 on given X Y Coordinates of screen.
		TouchAction action3 = new TouchAction((MobileDriver) driver).longPress(
				x3, y3).waitAction();
		// Set touch action4 on given X Y Coordinates of screen.
		TouchAction action4 = new TouchAction((MobileDriver) driver).longPress(
				x4, y4).waitAction();
		// Set touch action5 on given X Y Coordinates of screen.
		TouchAction action5 = new TouchAction((MobileDriver) driver).longPress(
				x5, y5).waitAction();//waitAction(1500);

		// Generate multi touch action chain using different actions and perform
		// It.
		maction.add(action1).add(action2).add(action3).add(action4)
				.add(action5).perform();
	}
