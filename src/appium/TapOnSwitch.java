...
//		driver.scrollTo("Views");
		...
		driver.findElementsByAccessibilityId("Views");
		// Scroll till element which contains "Switches" text If It Is not
		// visible on screen.
//		driver.scrollTo("Switches");
		// Click on Switches.
//		driver.findElementsByXPath("//android.widget.TextView[text='Switches']").click();

		// Get status of switch using it's text.
		String switchStatus1 = driver.findElementById(
				"io.appium.android.apis:id/monitored_switch").getText();
		...
		if ((switchStatus1.trim()).equals("Monitored switch OFF")) {
			...
			MobileElement swt = (MobileElement) driver
					.findElementByName("Monitored switch OFF");
			// Tap on switch button "Monitored switch OFF" to make it ON.
//			swt.tap(1, 1);
			TouchActions action = new TouchActions(driver);
			action.singleTap(driver.findElementByXPath("//"));
			action.perform();
			action.singleTap(driver.findElement(By.xpath(""))).perform();
		...
		// Get status of switch using it's text.
		String switchStatus2 = driver.findElementById(
				"io.appium.android.apis:id/monitored_switch").getText();
		...
		if ((switchStatus2.trim()).equals("Monitored switch ON")) {
			...
			MobileElement swt = (MobileElement) driver
					.findElementByName("Monitored switch ON");
