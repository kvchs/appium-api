...
//		Boolean iselementpresent1 = driver.findElements(By.xpath("//android.widget.TextView[text='App']")).size() != 0;
		Boolean iselementpresent = driver.findElementsByAccessibilityId("App").size() != 0;
...
//		driver.findElementByAndroidUIAutomator("new UiSelector().text(\"+<your_text>+\")");
		Assert.assertTrue(iselementpresent,
				"Targeted element App is not present on screen");
	...
	public void checkLoaderElementPresent() {
		...
		Boolean iselementpresent = driver.findElementsByXPath("//android.widget.TextView[text='Loader']").size() != 0;
		// iselementpresent will be false so assertion will fail and so test
		// method will fail too.
		Assert.assertTrue(iselementpresent,
				"Targeted element Loader is not present on screen");
